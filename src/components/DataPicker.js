		...
		<div id="runsWrapper">
			{props.data.slice().sort((a, b) => b.startTime - a.startTime).map(run => (
				...
				>
					...
					<span className="letter" title="Identifier">{run.letter === null || run.letter === "0" ? run.runname : run.letter}</span>
...
function Selections(props) {
	...
	function formatWorkloadLabel(workloadobj) {
		let workload = workloadobj.workload;
		if (workload === "null") {
			workload = "Unsorted Runs";
		}
		else {
			if (workloadobj.runs.length === 1) {
				workload = "Workload " + workloadobj.runs[0].experimentId + "-" + workloadobj.runs[0].runname;
			} else {
				workload = "Workload " + workload;
			}
		}
			...
			{visibleSelection.map(visibleWorkload => (
				...
				>
					<div className='workload'>
						...
						{formatWorkloadLabel(visibleWorkload)}
					</div>
					<ul>
						{ /* render all runs */}
						{visibleWorkload.runs.sort((a, b) => a.startTime - b.startTime).map(visibleRun => (
							<li key={visibleRun.name}>
								...
								{visibleRun.letter === null || visibleRun.letter === "0" ? visibleRun.runname : visibleRun.letter}
